...
async def register(message: qq.Message):
    ...
    userBasicInfo = pandas.DataFrame([[20, 'Special', -1]], columns = ['weaponAttack', 'weaponRarity', 'weaponRarityRaw'], index = ['新手剑'])
    userBasicInfo.to_json('./users/' + user + '_weaponForm.json', indent = 4, orient = 'index')
    userBasicInfo = pandas.DataFrame([[1, 'Common', 0.4]], columns = ['itemAmount', 'itemRarity', 'itemRarityRaw'], index = ['生命药水'])
