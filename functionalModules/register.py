...
async def Register(message: qq.Message):
    ...
    #basic_info
    pandas.Series(index = ['sky_dust_amount', 'signed_days', 'last_activity_year', 'last_activity_month', 'last_activity_day', 'earth_dust_amount', 'continuous_signed'], data = [0, 0, 1, 1, 1, 0, 0]).to_json('./Users/' + user + '_basic_info.json', indent = 4)
    #weapon_form
    ...
    #item_form
    pandas.DataFrame(data= [[0, 0, 0, 0], [1, 'Common', 0.4, 2]], columns = ['item_amount', 'item_rarity', 'item_rarity_raw', 'item_step'], index = [0, '生命药水']).to_json('./Users/' + user + '_item_form.json', indent = 4, orient = 'index')
    #armor_form
